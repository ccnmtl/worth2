...
class SupporterReportColumnTest(TestCase):
    ...
    def test_description(self):
        ...
        self.assertEqual(column.description(), 'Supporter 1 Closeness')

    def test_identifier(self):
        column = SupporterReportColumn(0, 'closeness', 'single choice')
        self.assertEqual(column.identifier(), 'supporter_1_closeness')

    def test_metadata(self):
        column = SupporterReportColumn(0, 'closeness', 'single choice',
                                       'VC', 'Very Close')
        self.assertEqual(column.metadata(), ['', 'supporter_1_closeness',
    ...
    def test_user_value_no_supporters(self):
        ...
        column = SupporterReportColumn(0, 'closeness', 'single choice')
        self.assertEqual(column.user_value(participant), '')
    ...
    def test_user_value_supporters(self):
        ...
        column = SupporterReportColumn(0, 'closeness', 'single choice')
        self.assertEqual(column.user_value(participant), 'VC')
        column = SupporterReportColumn(0, 'influence', 'single choice')
        self.assertEqual(column.user_value(participant), 'P')
        column = SupporterReportColumn(0, 'provides_emotional_support',
                                       'string')
